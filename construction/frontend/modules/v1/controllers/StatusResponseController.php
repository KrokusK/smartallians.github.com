...
use frontend\modules\v1\models\StatusResponse;
use Yii;
        ...
        if (is_array($bodyRaw)) {
            if (array_key_exists('StatusResponse[id]', $bodyRaw)) {
                ...
                foreach ($bodyRaw as $name => $value) {
                    ...
                    if (strtolower(substr($name, 0, $pos_begin - 1)) != 'statusresponse') return Json::encode(array('method' => 'POST', 'status' => '1', 'type' => 'error', 'message' => 'Ошибка валидации: '.$name));
                    $pos_end = strpos($name, ']');
                ...
                if (array_key_exists('StatusResponse[id]', $bodyRaw)) {
                    ...
                    if (!empty($modelStatusResponse)) {
                        ...
                        foreach ($bodyRaw as $name => $value) {
                            ...
                            $pos_begin = strpos($name, '[') + 1;
                            if (strtolower(substr($name, 0, $pos_begin - 1)) != 'statusresponse') return Json::encode(array('method' => 'PUT', 'status' => '1', 'type' => 'error', 'message' => 'Ошибка валидации: '.$name));
                            $pos_end = strpos($name, ']');
                            $name = substr($name, $pos_begin, $pos_end - $pos_begin);
