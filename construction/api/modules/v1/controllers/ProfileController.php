...
class ProfileController extends Controller
{
    ...
    public function actionCreate()
    {
        ...
        //$array_put = $this->parsingProfileFormData($put_string);

        $bodyRaw = json_decode(Yii::$app->getRequest()->getRawBody(), true);
        //$body = json_decode(Yii::$app->getRequest()->getBodyParams(), true);

        //$modelProfile->setAttributes($bodyRaw);

        // load attributes in Profile object
        // example: yiisoft/yii2/base/Model.php
        if (is_array($bodyRaw)) {
            if (array_key_exists('Profile[id]', $bodyRaw)) {
                ...
                foreach ($bodyRaw as $name => $value) {
                    ...
                    if (strtolower(substr($name, 0, $pos_begin - 1)) != 'profile') return Json::encode(array('method' => 'POST', 'status' => '1', 'type' => 'error', 'message' => 'Ошибка валидации: '.$name));
                    $pos_end = strpos($name, ']');
                    $name = substr($name, $pos_begin, $pos_end-$pos_begin);
                    //if (isset($modelProfile->$name)) {
                    //    $modelProfile->$name = $value;
                    //}
        ...
        if ($modelProfile->validate()) {
            ...
            try {
                $flag = $modelProfile->save(false); // insert
            ...
            //return Json::encode(array('method' => 'POST', 'status' => '0', 'type' => 'success', 'message' => 'Профиль успешно сохранен', var_dump($bodyRaw), var_dump(ArrayHelper::toArray($modelProfile))));
        ...
        //if (Yii::$app->request->isAjax) {
            ...
            if (is_array($bodyRaw)) {
                if (array_key_exists('Profile[id]', $bodyRaw)) {
                    ...
                    if (!preg_match("/^[0-9]*$/",$bodyRaw['Profile[id]'])) {
                        return Json::encode(array('method' => 'PUT', 'status' => '1', 'type' => 'error', 'message' => 'Ошибка валидации: id'));
                    }

                    // Search record by id in the database
                    $query = Profile::find()
                        ->where(['id' => $bodyRaw['Profile[id]']]);
                    //->where(['AND', ['id' => $modelProfile->id], ['user_desc_id'=> $var2]]);

                    $modelProfile = $query->orderBy('created_at')->one();

                    if (!empty($modelProfile)) {
                        // update in the properties in the Profile object
                        foreach ($bodyRaw as $name => $value) {
                            $pos_begin = strpos($name, '[') + 1;
                            if (strtolower(substr($name, 0, $pos_begin - 1)) != 'profile') return Json::encode(array('method' => 'PUT', 'status' => '1', 'type' => 'error', 'message' => 'Ошибка валидации: '.$name));
                            $pos_end = strpos($name, ']');
                            $name = substr($name, $pos_begin, $pos_end - $pos_begin);
            ...
            if ($modelProfile->validate()) {
                ...
                try {
                    ...
                    if ($flag == true) {
                        ...
                        $transaction->rollBack();
                        return Json::encode(array('method' => 'PUT', 'status' => '1', 'type' => 'error', 'message' => 'Ошибка: Профиль не может быть сохранен (обновлен)'));
                    }
                } catch (Exception $ex) {
                    $transaction->rollBack();
                    return Json::encode(array('method' => 'PUT', 'status' => '1', 'type' => 'error', 'message' => 'Ошибка: Профиль не может быть сохранен (обновлен)'));
                ...
                //return Json::encode(array('method' => 'PUT', 'status' => '0', 'type' => 'success', 'message' => 'Профиль успешно сохранен (обновлен)', var_dump($bodyRaw), var_dump(ArrayHelper::toArray($modelProfile))));
                return Json::encode(array('method' => 'PUT', 'status' => '0', 'type' => 'success', 'message' => 'Профиль успешно сохранен (обновлен)'));
            } else {
    ...
    public function actionDelete()
    {
        ...
        if (!empty($modelProfile)) {
            $transaction = \Yii::$app->db->beginTransaction();
            try {
                $flag = $modelProfile->delete($bodyRaw['Profile[id]']); // delete

                if ($flag == true) {
                    $transaction->commit();
                } else {
                    $transaction->rollBack();
                    return Json::encode(array('method' => 'PUT', 'status' => '1', 'type' => 'error', 'message' => 'Ошибка: Профиль не может быть удален'));
                }
            } catch (Exception $ex) {
                $transaction->rollBack();
                return Json::encode(array('method' => 'PUT', 'status' => '1', 'type' => 'error', 'message' => 'Ошибка: Профиль не может быть удален'));
            ...
            //return Json::encode(array('method' => 'PUT', 'status' => '0', 'type' => 'success', 'message' => 'Профиль успешно удален', var_dump($bodyRaw), var_dump(ArrayHelper::toArray($modelProfile))));
            return Json::encode(array('method' => 'PUT', 'status' => '0', 'type' => 'success', 'message' => 'Профиль успешно удален'));
        } else {
            return Json::encode(array('method' => 'PUT', 'status' => '1', 'type' => 'error', 'message' => 'Ошибка: Профиль не может быть удален'));
        }
        //}
     ...
     * @return array
     */
    public function parsingProfileFormData($put_string)
    {
