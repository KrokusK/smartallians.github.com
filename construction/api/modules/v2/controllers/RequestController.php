                    ...
                    if ($modelValidate->hasAttribute($nameRequestAssoc)) {
                        if ($nameRequestAssoc == 'description') {
                            ...
                            if (is_array($getParams[$arrayRequestAssoc[$nameRequestAssoc]])) {
                                ...
                                if (!$modelValidate->validate($nameRequestAssoc)) return Json::encode(array('method' => 'GET', 'status' => 1, 'type' => 'error', 'message' => $getParams[$arrayRequestAssoc[$nameRequestAssoc]].'Ошибка валидации: параметр '.$valueRequestAssoc));
