                    ...
                    if ($modelValidate->hasAttribute($nameRequestAssoc)) {
                        if ($nameRequestAssoc == 'description') {
                            ...
                            if (is_array($getParams[$arrayRequestAssoc[$nameRequestAssoc]])) {
                                ...
                                if (!$modelValidate->validate($nameRequestAssoc)) return Json::encode(array('method' => 'GET', 'status' => 1, 'type' => 'error', 'message' => var_dump($getParams[$arrayRequestAssoc[$nameRequestAssoc]][0]).' - '.' - '.'Ошибка валидации: параметр '.$valueRequestAssoc));
